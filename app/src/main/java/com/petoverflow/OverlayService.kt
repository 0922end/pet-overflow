...
.ey-line{width:12px;height:2.5px;background:#1565C0;border-radius:2px;position:absolute;top:6px}
.ey-angry{width:12px;height:2.5px;background:#1565C0;position:absolute;top:5px;left:1px;transform:rotate(-15deg)}
.ey-sad{width:12px;height:8px;border-top:2.5px solid #1565C0;border-radius:50% 50% 0 0;position:absolute;top:5px}
.ey-close{width:10px;height:2.5px;background:#1565C0;border-radius:2px;position:absolute;top:6px;left:1px}
...
.mo-nm{top:30px;width:7px;height:3px;border-bottom:2px solid #F48FB1;border-radius:0 0 50% 50%}
.mo-hp{top:29px;width:9px;height:5px;border-bottom:2px solid #F48FB1;border-radius:0 0 50% 50%}
.mo-cr{top:31px;width:6px;height:4px;border-bottom:2px solid #F48FB1;border-radius:0 0 50% 50%}
.mo-ks{top:29px;width:7px;height:7px;border:2px solid #F48FB1;border-radius:50%}
.mo-tg{top:30px;width:7px;height:5px;background:#F48FB1;border-radius:0 0 5px 5px}
.mo-ag{top:30px;width:8px;height:3px;border-bottom:2px solid #F48FB1;border-radius:0 0 50% 50%}
.mo-sd{top:32px;width:6px;height:3px;border-top:2px solid #F48FB1;border-radius:50% 50% 0 0}
.mo-wk{top:30px;width:7px;height:3px;border-bottom:2px solid #F48FB1;border-radius:0 0 50% 50%}

/* 胡须：从嘴角(脸两侧中间偏下)向两侧发散 */
/* 左脸胡须(用户右侧)：从右边界向左延伸，旋转中心在右端 */
.wl{position:absolute;height:1px;background:#d4bfb0;z-index:0;transform-origin:right center;right:10px}
.wl1{width:15px;top:25px;transform:rotate(18deg)}
.wl2{width:17px;top:28px;transform:rotate(0deg)}
.wl3{width:15px;top:31px;transform:rotate(-18deg)}
/* 右脸胡须(用户左侧)：从左边界向右延伸，旋转中心在左端 */
.wr{position:absolute;height:1px;background:#d4bfb0;z-index:0;transform-origin:left center;left:10px}
.wr1{width:15px;top:25px;transform:rotate(-18deg)}
.wr2{width:17px;top:28px;transform:rotate(0deg)}
.wr3{width:15px;top:31px;transform:rotate(18deg)}
...
/* 项圈 - 简单蓝色弧线 */
.col{position:absolute;top:42px;left:50%;transform:translateX(-50%);width:24px;height:5px;background:#42A5F5;border-radius:3px;opacity:0;transition:opacity .3s;z-index:1}
.col.on{opacity:1}
...
<div class="wr wr1"></div><div class="wr wr2"></div><div class="wr wr3"></div>
...
function setEyes(type){
  EL.innerHTML='';ER.innerHTML='';EL.className='el';ER.className='er'
  var ehtml='<div class="ey" id="EL"><div class="es"></div></div>'
  var html=''
  if(type===0){html='<div class="ey" id="EL"><div class="es"></div></div>'}
  else if(type===1){html='<div class="ey-arc"></div>'}
  else if(type===2){html='<div class="ey-sad"></div>'}
  else if(type===3){html='<div class="ey-arc"></div>'}
  else if(type===4){html='<div class="ey" id="EL"><div class="es"></div></div>'}
  else if(type===5){EL.innerHTML='<div class="ey" id="EL"><div class="es"></div></div>';ER.innerHTML='<div class="ey-close"></div>';return}
  else if(type===6){html='<div class="ey-angry"></div>'}
  else if(type===7){html='<div class="ey-sad"></div>'}
  EL.innerHTML=html;ER.innerHTML=html
}

function ex(n){
  if(n===0){setEyes(0);MO.className='mo mo-nm'}
  else if(n===1){setEyes(1);MO.className='mo mo-hp'}
  else if(n===2){setEyes(2);MO.className='mo mo-cr'}
  else if(n===3){setEyes(3);MO.className='mo mo-ks'}
  else if(n===4){setEyes(4);MO.className='mo mo-tg'}
  else if(n===5){setEyes(5);MO.className='mo mo-wk'}
  else if(n===6){setEyes(6);MO.className='mo mo-ag'}
  else if(n===7){setEyes(7);MO.className='mo mo-sd'}
}
ex(0);
